...
class Controller(object):
    def __init__(self, logger):
        ...
        self.res1 = [None, None, None, None, None]
        self.res2 = [None, None, None, None, None]
    ...
    def run(self):
        ...
        chufazongshu = 0
        while True:
            result_det_1 = [1, 1, 1, 1]
            if self.ret_sender.qsize() >= 12:
                # result_det = [0, 0, 0, 0, 0, 0, 0, 0]
                ...
                while self.ret_sender.qsize() > 0:
                    ...
                    print('222222======', batch_count)
                    # print('canid',cam_ret['camId'])
                    ...
                    if camID == 0:
                        ...
                        if idx == 0:
                            print('1111----', self.result_b[1])

                            result_det_1[1] = result_det_1[1] * defect
                        elif idx == 1:
                            print('0000----', result_det_1[0])

                            result_det_1[0] = result_det_1[0] * defect
                        elif idx == 2:
                            print('3333----', result_det_1[3])

                            result_det_1[3] = result_det_1[3] * defect
                        elif idx == 3:
                            print('2222----', result_det_1[2])

                            result_det_1[2] = result_det_1[2] * defect
                        print('de-----', defect)
                        print('b-----', result_det_1)
                        # result_det_1 = self.result_b
                print('-----------------start-----------------------')
                chufazongshu = chufazongshu + 1
                print('chufazongshu: ', chufazongshu)
                print('plc_shifoushuchu: ', self.capture_done)
                if self.capture_done == True:
                    # print('输出结果', result_det)
                    now_time = datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S_%f')
                    print('dangqian time: ', now_time)
                    print('trig_count: ', trig_count)
                    print('step', step)
                    print('result_det_0', result_det_0)
                    print('result_det_1', result_det_1)
                    red = self.plc_client.execute(1, cst.READ_HOLDING_REGISTERS, 12000, 4)  # 这里可以修改需要读取的功能码
                    print('plcData: ', red)
                    zongshu_num = int(self.redis_db.get('zongshu_num'))
                    liangpin_num = int(self.redis_db.get('liangpin_num'))
                    if trig_count > 5:
                        for j in range(len(self.res1[step])):
                            if self.res1[step][j] == 0 or self.res2[step][j] == 0:
                                self.res1[step][j] = 0
                        for i in self.res1[step]:
                            zongshu_num += 1

                            if i == 1:
                                liangpin_num += 1
                        self.redis_db.set('zongshu_num', zongshu_num)
                        self.redis_db.set('liangpin_num', liangpin_num)
                        print('myyData:', self.res1[step])
                    # 将相机1结果放入res1
                    self.res1[step] = result_det_0
                    # 将相机2结果放入res2
                    if step - 1 < 0:
                        self.res2[4] = result_det_1
                    elif trig_count > 1:
                    ...
                    try:
                        ...
                        # result0 = self.plc_client.execute(1, cst.READ_HOLDING_REGISTERS, 10001, 4)
                        # result1 = self.plc_client.execute(1, cst.READ_HOLDING_REGISTERS, 11001, 4)
                        #
                        # print("result0:", result0, "result1:", result1)
                        i_duxie = i_duxie + 1
                        result_det = [i_duxie, i_duxie, i_duxie, i_duxie]
